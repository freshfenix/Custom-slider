...
- **ОБОВ'ЯЗКОВО**: Реалізуйте наступні обробники подій:
  ...
  - Натискання кнопки паузи: `pausePlayHandler`
  - Натискання кнопки вперед: `nextHandler`
  - Натискання кнопки назад: `prevHandler`
  - Натискання на індикатор: `indicatorClickHandler`
  - Натискання клавіші: `keydownHandler`
  - Свайп (початок): `swipeStartHandler`
  - Свайп (кінець): `swipeEndHandler`

- **ОБОВ'ЯЗКОВО**: Призначайте обробники подій на наступні елементи:
...
- **ОБОВ'ЯЗКОВО**: Підтримка свайпів для настільних (миша) та сенсорних пристроїв (touch)
...
**ВАЖЛИВО:** Змінені назви змінних або функцій у порівнянні з вимогами або неправильне призначення обробників подій можуть призвести до невдалого проходження тестів. Дотримуйтесь вказаних назв та структури коду.
